...
using namespace std;
int main(void){
    //1) 다 더하고 > 갯수 > /100 0.. 0index 1.. 1index > 차이 구하기 >> dump횟수 고려안됨
    //2 ) 매 횟수마다 ** 덤프횟수동안 > 가장 큰 값 -1 & 작은 값 +1 (매번 최댓값 최솟값 탐색 ,, 분기 )
    int iDumpNo;
    int iMin, iMax;
    int iMinIndex, iMaxIndex, iRes;

    int iHeight[ARRSIZE]; // 각 상자의 높이값
    //test case 10개
    for(int iLoop=0; iLoop<TESTCASE; ++iLoop)
    {

        cin >> iDumpNo;
        for(int iInput = 0; iInput<ARRSIZE; ++iInput){
            cin >> iHeight[iInput];
        }

        for(int j=0; j<iDumpNo; ++j){ // 덤프횟수만큼 진행
            iMin = iHeight[0];
            iMax = iHeight[0];
            ...
            for(int iMinSearch = 1; iMinSearch<ARRSIZE; ++iMinSearch){
                if(iMin > iHeight[iMinSearch]){
                    iMin = iHeight[iMinSearch];
                    iMinIndex = iMinSearch;
                }
            }
            for(int iMaxSearch = 1; iMaxSearch<ARRSIZE; ++iMaxSearch){
                if(iMax < iHeight[iMaxSearch]){
                    iMax = iHeight[iMaxSearch];
                    iMaxIndex = iMaxSearch;
                }
            }
            iRes = iMax - iMin;
            if(iRes <= 1){
        ...
        for(int iSearch = 1; iSearch < ARRSIZE; ++iSearch)
        {
            if(iMin > iHeight[iSearch])
            {
                iMin = iHeight[iSearch];
            }

            if(iMax < iHeight[iSearch])
            {
                iMax = iHeight[iSearch];
            }
        }
